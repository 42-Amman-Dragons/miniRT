...
/*   Updated: 2026/08/09 16:01:29 by hal-lawa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color lighting(t_material material, t_light light, t_tuple point, t_tuple eyev, t_tuple normal)
{
    ...
    if(dot_product(lightv, normal) < 0)
        ...
        amb_diff_spect[1] = scale_color(scale_color(effective_color, material.diffuse),
            dot_product(lightv, normal));
        reflectv = reflect(negate_tuple(lightv), normal);
        if(dot_product(reflectv, eyev) <= 0)
            amb_diff_spect[2] = new_color(0,0,0);
        else
        {
            amb_diff_spect[2] = scale_color(scale_color(light_intensity, material.specular),
                pow(dot_product(reflectv, eyev), material.shininess)
            );
        }
    }
